...
if ($price_max)
{
    $where = $where . " AND `rent_price_seller`<=$price_max";
}

if ($price_min)
{
    $where = $where . " AND `rent_price_seller`>=$price_min";
}
...
if (isset($auth_team_id))
{
    $sql = "SELECT `city_name`,
            ...
            LEFT JOIN `rent`
            ON `rentapplication_rent_id`=`rent_id`
            LEFT JOIN `player`
            ON `rent_player_id`=`player_id`
            LEFT JOIN `name`
            ON `player_name_id`=`name_id`
...
foreach ($rentapplication_array as $item)
{
    $player_id[] = $item['player_id'];
}
