...
/**
 * Формуємо таблицю кубка міжсезоння і ігри
 */
function f_igosja_newseason_offseason()
{
    ...
    $sql = "INSERT INTO `offseason` (`offseason_season_id`, `offseason_team_id`)
            SELECT $igosja_season_id+1, `team_id`
            FROM `team`
            WHERE `team_id`!=0
    ...
    $sql = "UPDATE `offseason`
            ...
            AND `offseason_season_id`=$igosja_season_id+1";
    f_igosja_mysqli_query($sql);
    ...
    $sql = "SELECT `offseason_team_id`,
            ...
            WHERE `offseason_season_id`=$igosja_season_id+1
            ORDER BY RAND()";
